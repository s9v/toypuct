...
int main()
{
	...
	{
		cin >> req[i].first;
		cin >> req[i].second.first;
		req[i].second.second = i;
	}
	...
	{
		for (int j = 0; j < k; j++)
		if (v[j] == -1 && req[i].first <= r[j].first)
		{
			v[j] = i;
			s += req[i].second.first;
			m++;
			break;
		}
	}
	...
	for (int i = 0; i < k; i++)
		if (v[i] != -1)
		{
			cout << req[v[i]].second.second+1 << " " << r[i].second+1 << "\n";
		}

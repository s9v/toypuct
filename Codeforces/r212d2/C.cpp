...
int bigger(int x, int l, int r) {
	return big[x][r] - big[x][l-1];
}
int smaller(int x, int l, int r) {
	return (r-l+1)-bigger(x,l,r);
}
	...
	for (int i = 1; i <= n; i++)
		for (int j = i+1; j <= n; j++) {
			...
			if (j-i == 1)
				...
				v =  + bigger(i,i+1,j) - smaller(i,i+1,j) - bigger(j,i+1,j-1) + smaller(j,i+1,j-1);
			
//			printf("ifswap %d %d increase %d\n", i, j, v);
//			printf("ifswap %d %d swaps %d\n\n", i, j, work+v);

...
int main()
{
	...
	for (int i = 0; i < n; i++)
	for (int j = 0; j < m; j++)
	if ((xb[j] <= xa[i] && xa[i] <= xb[j]+lb[j]) && (ya[i] <= yb[j] && yb[j] <= ya[i]+la[i])) {
		int res = abs(xa[i]-xb[j]);
		res = min(res, abs(xa[i]-(xb[j]+lb[j])));
		res = min(res, abs(ya[i]-yb[j]));
		res = min(res, abs(ya[i]+la[i]-yb[j]));

...
//	freopen("output.txt", "w", stdout);
	...
	while( scanf("%c",&c)!=EOF ) s[ln++] = c;
	
	REP(i,ln)
	{
		if( i<ln-3 && ( s[i]=='t' && s[i+1]=='r' && s[i+2]=='a' && s[i+3]=='m' ) && (i-1<0 || !isalpha(s[i-1])) && (i+4>ln || !isalpha(s[i+4])) )
		tram++;
		
		if( i<ln-9 &&
		(
		s[i]  =='t' && s[i+1]=='r' && s[i+2]=='o' && s[i+3]=='l' && s[i+4]=='l' &&
		s[i+5]=='e' &&s[i+6]=='y' && s[i+7]=='b' &&s[i+8]=='u' && s[i+9]=='s'
		)
		&& (i-1<0 || !isalpha(s[i-1])) && (i+10>ln || !isalpha(s[i+10])) )
		trol++;
	}
